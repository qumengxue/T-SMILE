...
class VSTGCheckpointer(object):
    ...
    def load(self, f=None, with_optim=True, load_mapping={}, retrain_f=None):
        ...
        if self.cfg.MODEL.RETRAIN:
            retrain_checkpoint = self._load_file(retrain_f)
            # temporal_encoder_dict = {k: v for k, v in retrain_checkpoint["model_ema"].items() if 'ground_encoder.encoder.temporal_layers' in k}
            temporal_encoder_dict = {k: v for k, v in retrain_checkpoint["model_ema"].items() if 'ground_encoder.encoder' in k}
            self.model.load_state_dict(temporal_encoder_dict, strict=False)
            self.model_ema.load_state_dict(deepcopy(self.model).state_dict(), strict=False)
