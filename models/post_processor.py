...
class PostProcess(nn.Module):
    ...
    def forward(self, outputs, target_sizes, frames_id, durations):
        ...
        for i_b in range(b):
            ...
            #########pred sted with actioness#################
            prob_action = outputs["pred_actioness"].to(torch.double)
            action_thresh = torch.where(prob_action < 0.7, 0., prob_action)
            if action_thresh.sum() != 0:
                start_idx = torch.nonzero(action_thresh.squeeze())[0]
                end_idx = torch.nonzero(action_thresh.squeeze())[-1]
            else:
                start_idx = start_idx
                end_idx = end_idx
            ####################################################
