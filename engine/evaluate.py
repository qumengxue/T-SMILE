...
def single_forward(cfg, model, videos, texts, targets, device, postprocessor):
    ...
    return bbox_pred, temp_pred, outputs["pred_actioness"], outputs["pred_sted"]
...
def do_eval(cfg, mode, logger, model, postprocessor, data_loader, evaluator, device):
    ...
    for _, batch_dict in enumerate(tqdm(data_loader)):
        ...
        bbox_pred1, temp_pred1, action_pred1, sted_pred1 = single_forward(cfg, model, videos1, texts, 
                                targets1, device, postprocessor)
        bbox_pred2, temp_pred2, action_pred2, sted_pred2 = single_forward(cfg, model, videos2, texts, 
                                targets2, device, postprocessor)
        
        bbox_pred, temp_pred, action_pred, sted_pred = {}, {}, {}, {}
        for vid in bbox_pred1:
            ...
            pred_actioness = torch.zeros(action_pred1.shape[0], action_pred1.shape[1]+action_pred2.shape[1], action_pred1.shape[-1])
            pred_actioness[:,0::2,:] = action_pred1
            pred_actioness[:,1::2,:] = action_pred2
            action_pred[vid] = pred_actioness
            
            pred_sted = torch.zeros(sted_pred1.shape[0], sted_pred1.shape[1]+sted_pred2.shape[1], sted_pred1.shape[-1])
            pred_sted[:,0::2,:] = sted_pred1
            pred_sted[:,1::2,:] = sted_pred2
            sted_pred[vid] = pred_sted
            
        evaluator.update(bbox_pred)
        evaluator.video_update(temp_pred)
        evaluator.acioness_update(action_pred)
        evaluator.sted_update(sted_pred)
