...
class ActivityiouEvaluator:
    def __init__(
        ...
        assert subset in ["train", "test"], f"Wrong Activity subset {subset}"
    
        gt_data = []
        cache_dir = os.path.join(vidstg_path, 'data_cache')
        dataset_cache = os.path.join(cache_dir, f'activity-{subset}-anno.cache')
...
class ActivityEvaluator(object):
    def __init__(
        self,
        logger,
        vidstg_path,
        ...
        """
        self.evaluator = ActivityiouEvaluator(
            vidstg_path,
            subset=subset,
    ...
    def acioness_update(self, pred_actioness):
        self.pred_actioness.update(pred_actioness)
    
    def sted_update(self, pred_sted):
        self.pred_sted.update(pred_sted)
