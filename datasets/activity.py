...
from .data_utils import SENTENCE_SPLIT_REGEX, make_activity_input_clip
...
class ActivityDataset(data.Dataset):
    ...
    def load_frames(self, data_item, load_video=True):
        ...
        if load_video:
            ...
            origin_frame_num = frames.shape[0]
        ...
        return frames, origin_frame_num
    ...
    def __getitem__(self, index: int):
        ...
        data_item = make_activity_input_clip(self.cfg, self.split, video_data)
        
        frames, origin_frame_num = self.load_frames(data_item)   # T * C * H * W
        ...
        if 'track_bboxs' in video_data:
            bbox_idx_origin = [frame_ids[idx] for idx in range(start_idx,end_idx + 1)]
            bbox_idx = [min(video_data["track_frame_id"].index(box_id), len(video_data['track_bboxs'])-1) for box_id in bbox_idx_origin]
        ...
        # sentence = data_item['description'] + ". Find the " + data_item['object'] + "."
        # sentence = "Find the "+ data_item['object'] +", "+ data_item['description']
        sentence = data_item['object'] +": "+ data_item['description']
    ...
    def load_data(self):
        ...
        dataset_cache = os.path.join(cache_dir, f'activity-{self.split}-input.cache')
        # Used For Evaluateion
        gt_anno_cache = os.path.join(cache_dir, f'activity-{self.split}-anno.cache')
